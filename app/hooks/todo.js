        ...
        if(program && publicKey) {
            try {
                ...
                if(input) {
                    ...
                    toast.success('Succesfully added todo')
                }
            } catch (error) {
                console.log(error)
                toast.error(error.toString())
    ...
    const removeTodo = async (todoPda,todoIdx) => {
        if(program && publicKey) {
            try {
                ...
                setLoading(true)
                const [profilePda,profileBump] = findProgramAddressSync([utf8.encode('USER_STATE'),publicKey.toBuffer()],program.programId)
                await program.methods.removeTodo(todoIdx).accounts({userProfile:profilePda,todoAccount:todoPda,authority:publicKey,systemProgram:SystemProgram.programId}).rpc()
